...
public class ARSCDecoder {
	...
	private void readEntry() throws IOException, AndrolibException {
        ...
        if (value instanceof ResColorValue && newColorMap.containsKey(spec.getName())) {
            // 替换最后的四个bit为新的color值
            int newColor = hex2Decimal(newColorMap.get(spec.getName()));
            mOut.remove(4);
            mOut.writeIntByBigEndian(newColor);
        }
    ...
    private int hex2Decimal(String hex) {
        ...
        int a = 255, r = (i >> 16) & 0xFF, g = (i >> 8) & 0xFF, b = i & 0xFF;
        return ((a & 0xFF) << 24) |
                ((r & 0xFF) << 16) |
                ((g & 0xFF) << 8)  |
                ((b & 0xFF) << 0);
	...
	public static Map<String, String> newColorMap = new HashMap<String, String>();
	public static LittleEndianDataOutputStream mOut;
